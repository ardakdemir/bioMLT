...
then
    ...
    if [ $question_type = 'yesno' ]
    then
        singularity exec --nv ~/singularity/pt-cuda-tf python biomlt.py  --squad_dir .  --biobert_model_path $pretrained_biobert_model_path --model_save_name $model_save_name --output_dir $output_dir  --num_train_epochs $epoch_num  --overwrite_cache --load_model --load_model_path $finetuned_model --squad_yes_no 
    else
        singularity exec --nv ~/singularity/pt-cuda-tf python biomlt.py  --squad_dir .  --biobert_model_path $pretrained_biobert_model_path --model_save_name $model_save_name --output_dir $output_dir  --num_train_epochs $epoch_num  --overwrite_cache --load_model --load_model_path $finetuned_model 
    fi
...
do
    ...
    if [ $question_type = 'yesno' ]
    then
        singularity exec --nv ~/singularity/pt-cuda-tf python biomlt.py --predict --load_model_path $load_model_path  --squad_dir . --nbest_path $nbest_path --load_model --squad_yes_no --squad_predict_yesno_file $squad_predict_file --overwrite_cache --pred_path $pred_path
    else
        singularity exec --nv ~/singularity/pt-cuda-tf python biomlt.py --predict --load_model_path $load_model_path  --squad_dir . --nbest_path $nbest_path --load_model  --squad_predict_factoid_file $squad_predict_file --overwrite_cache --pred_path $pred_path
    fi
