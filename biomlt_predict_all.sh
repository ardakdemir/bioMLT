...
#$ -N  predict_all_factoid  -e predict_all_err  -o predidct_all_out
echo $EVAL_PATH
echo $BIOBERT_PATH
...
do
    ...
    singularity exec --nv ~/singularity/pt-cuda-tf python biomlt.py --predict --load_model_path $load_model_path  --squad_predict_factoid_file $squad_predict_file --squad_dir . --nbest_path $nbest_path --load_model  --mode qas
...
cat result_for_*.txt >> 'all_results_'${save_name}'.txt'
