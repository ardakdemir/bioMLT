...
singularity exec --nv ~/singularity/pt-cuda-tf python bioMLT/biomlt_alldata.py --init_ner_head --qas_with_ner --crf --mode ner --total_train_steps $total_train_steps --output_dir $output_dir  --num_train_epochs $epoch_num --ner_train_file  $target_train_file  --ner_dev_file $target_dev_file  --ner_test_file $target_test_file  --load_model
